...
@section('content')
    <div class="max-w-3xl">
        ...
        @else
            ...
            @if (($inbox['discussions'] ?? collect())->isNotEmpty())
                <h2 class="section-title mt-6 mb-2">Discussions assigned to you <span class="badge badge-flag ml-1">{{ $inbox['discussions']->count() }}</span></h2>
                <div class="card overflow-hidden">
                    <table class="data-table">
                        <thead><tr><th>Topic</th><th class="w-[160px]">Project</th><th class="w-[120px]">Due</th></tr></thead>
                        <tbody>
                        @foreach ($inbox['discussions'] as $d)
                            <tr>
                                <td class="text-gray-900">{{ $d->title }} @if($d->blocking)<span class="badge badge-danger ml-1">blocking</span>@endif</td>
                                <td class="text-gray-500 text-[13px]">{{ $d->project?->name }}</td>
                                <td class="text-gray-500 text-[13px]">{{ $d->due_at?->format('d/m/Y') ?? '—' }}</td>
                            </tr>
                        @endforeach
                        </tbody>
                    </table>
