...
@section('content')
    ...
    <div class="mb-6">
        ...
        <div class="card card-pad mt-4">
            @if ($session->phase === 'pre_analysis' && $canEdit)
                @if ($session->firewall_rejected_at)
                    <div class="mb-3 text-[13px] text-gray-600 bg-flag/10 border border-flag/30 rounded px-3 py-2">
                        Sent back at the quality firewall{{ $session->firewall_rejected_at ? ' ('.$session->firewall_rejected_at->diffForHumans().')' : '' }}:
                        <span class="font-medium">{{ $session->firewall_rejected_reason }}</span>
                    </div>
                @endif
            ...
            @elseif ($session->phase === 'firewall_review')
                @if ($canValidate)
                    ...
                    <form method="POST" action="{{ route('sessions.reject-firewall', $session) }}" class="mt-3 pt-3 border-t border-gray-100" x-data="{ open: false }">@csrf
                        <button type="button" class="btn-secondary" x-show="!open" @click="open = true">Send back for refinement…</button>
                        <div class="flex flex-wrap items-end gap-3" x-show="open" x-cloak>
                            <div class="flex-1 min-w-[240px]">
                                <label class="form-label">Reason (recorded, shown to the analyst)</label>
                                <input type="text" name="reason" maxlength="500" required class="form-input" placeholder="What must be improved before review…">
                            </div>
                            <button type="submit" class="btn-danger">Send back</button>
                        </div>
                    </form>
                @else
    ...
    <div class="mb-6">
        ...
        @include('partials.discussions', [
            ...
            'discussableKind' => 'session',
            'discussableId' => $session->id,
            'canEdit' => $canEdit,
        ])
    </div>
@endsection
